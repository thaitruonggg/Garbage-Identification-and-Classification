...
CLASSIFICATION_MODEL_PATH = 'models/ResnetModel.pth'  # Update with your classification model path

# Load the image
image_path = "external/test8.jpg"
...
# Initialize a list to hold all predicted classes
all_pred_classes = []
...
for idx in filtered_indices:
    ...
    # Add the predicted class and confidence to the list
    all_pred_classes.append(label)

    cv2.putText(img_cv, label, (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

# Print all the predicted classes for the objects in the image
print("The predicted classes for objects in the image", image_name, "are:")
...
plt.imshow(image)
plt.title("Predicted Classes: " + ", ".join(all_pred_classes))
plt.show()
